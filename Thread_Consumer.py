...
NTIS_Rawdata = NTIS_db.Rawdata

SCIENCEON_Author = SCIENCEON_db.collect
SCIENCEON_Rawdata = SCIENCEON_db.Rawdata
    ...
    while True:
        ...
        for msg in Consumer:        # ntis Author collection
            ...
            if r is not None:
                ...
                # 참여연구자 db저장
                rsc = re.sub('"', "", data["rsc"]).split(";")
                rscID = re.sub('"', "", data["rscId"]).split(";")
...
def go_scienceon(site, Consumer):
    while True :
        ...
        for msg in Consumer:             # scienceon Author collection
            data = msg
            key_id = data["keyId"]
            r = SCIENCEON_Rawdata.find_one({'$and': [{'id': data['id']}, {'keyId': key_id}]})                    # 질문
            if r is not None:
                continue
            else:
                a_id = []
                name = data["author"].split(";")
