...
class Consumer(threading.Thread):
    ...
    def __init__(self, site):
        ...
        dbs['public_QueryKeyword'] = db2.QueryKeyword
    ...
    def processing_rate(self, data, keyId, Author_info_Dic):         # 진행률, 진행 상태 함수
        ...
        if (self.pre_progress + 5 <= raw_progress ) and (raw_progress != 100):
            ...
            dbs['QueryKeyword'].update({"_id": keyId}, {'$set': {"progress": str(raw_progress)}})
            self.pre_progress = raw_progress
        elif (raw_progress == 100):
            # print(str(raw_progress))
            dt = datetime.datetime.now()
            Author_info_list = []
            count = dbs['Rawdata'].count({"keyId": keyId})
            idcount = dbs['AuthorPapers'].count()+1
            dbs['QueryKeyword'].update({"_id": keyId}, {'$set': {"progress": 0, "state": 1, "data": count, "crawl_time": dt.strftime("%Y-%m-%d %H:%M:%S")}})
        ...
        if self.site == 'NTIS' :
            ...
            if '03' in data['perfAgent'].split(',')[0] and mng_Name != "null" :
                ...
                if self.getA_Id(mng_ID) == 0:                      # Author에 저장되어 있는지 확인. 없으면 0, 있으면 해당 id
                    ...
                    dbs['Author'].insert_one(result)               # 책임 연구자 db넣기
    ...
    def run(self):
        ...
        while True :
            ...
            for msg in self.consumer:             # scienceon Author collection
                data = msg.value
                key_id = data['keyId']
                r = dbs['Rawdata'].find_one({'$and': [{'id': data['id']}, {'keyId': key_id}]})
